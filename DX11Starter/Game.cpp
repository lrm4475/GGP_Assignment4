...
void Game::Init()
{
	...
	//get size as next multiple of 16 (don't hardcode here)
	unsigned int size = sizeof(VertexShaderExternalData); 
	size = (size + 15) / 16 * 16; //will work even if struct size changes
...
void Game::CreateBasicGeometry()
{
	...
	Vertex sqVertices[] =
	{
		...
		{ XMFLOAT3(+0.4f, +0.3f, +0.0f), blue },
		{ XMFLOAT3(+0.4f, +0.1f, +0.0f), green },
		{ XMFLOAT3(+0.2f, +0.1f, +0.0f), red },
	};
	Vertex rhVertices[] =
	{
		{ XMFLOAT3(-0.5f, +0.2f, +0.0f), red },
		{ XMFLOAT3(-0.3f, +0.4f, +0.0f), blue },
	...
	unsigned int sqIndices[] = { 0, 1, 2, 0, 2, 3 };

	//make mesh objs
	unique_ptr<Mesh> triangle_M = make_unique<Mesh>(triVertices, 3, triIndices, 3, device);
	Entity triangle = Entity(std::move(triangle_M));
	Entity triangle2 = Entity(std::move(triangle_M));
	Entity triangle3 = Entity(std::move(triangle_M));
	unique_ptr<Mesh> square_M = make_unique<Mesh>(sqVertices, 4, sqIndices, 6, device);
	Entity square = Entity(std::move(square_M));
	unique_ptr<Mesh> rhombus_M = make_unique<Mesh>(rhVertices, 4, sqIndices, 6, device);
	Entity rhombus = Entity(std::move(rhombus_M));

	//add them to mesh vector
	entities.push_back(triangle);
	entities.push_back(triangle2);
	entities.push_back(triangle3);
	entities.push_back(square);
	entities.push_back(rhombus);
...
void Game::Draw(float deltaTime, float totalTime)
{
	...
	//Create local variable of cb struct
	//VertexShaderExternalData vsData;
	//vsData.colorTint = XMFLOAT4(0.5f, 1.0f, 0.5f, 1.0f);
	//vsData.worldMatrix = worldMatrix;
	//
	////bind resource to pipeline
	...
	for (int i = 0; i < entities.size(); i++) {
		//get world matrix
		Transform* currTrans = entities[i].GetTransform();

		//const buffer setting
		//Create local variable of cb struct
		VertexShaderExternalData vsData;
		vsData.colorTint = XMFLOAT4(0.5f, 1.0f, 0.5f, 1.0f);
		vsData.worldMatrix = currTrans->GetWorldMatrix();
